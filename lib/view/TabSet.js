...
/** @internal */
const TabSet = (props) => {
    const { node, layout, iconFactory, titleFactory, onTabMouseEnter, icons, path } = props;
    ...
    if (node.isEnableTabStrip()) {
        for (let i = 0; i < node.getChildren().length; i++) {
            ...
            tabs.push(React.createElement(TabButton_1.TabButton, { layout: layout, node: child, path: path + "/tb" + i, key: child.getId(), selected: isSelected, iconFactory: iconFactory, titleFactory: titleFactory, onTabMouseEnter: onTabMouseEnter, icons: icons }));
    ...
    const toolbar = (React.createElement("div", { key: "toolbar", ref: toolbarRef, className: cm(Types_1.CLASSES.FLEXLAYOUT__TAB_TOOLBAR), onMouseDown: onInterceptMouseDown, onTouchStart: onInterceptMouseDown, onDragStart: (e) => {
            e.preventDefault();
        } }, buttons));
    let header;
    let tabStrip;
